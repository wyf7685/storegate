...
| `copytree` / `movetree` | 默认 `overwrite=True`；`overwrite=False` 且目标已存在时抛 `FileExistsError`。Index、S3、DAV fallback 与 FTP 在覆盖写入时暂存被替换的目标；中途失败恢复已知目标内容、Index refs 以及 move 源，并把主错误置前保留回滚错误。 |
