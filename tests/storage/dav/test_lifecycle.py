...
from app.storage.dav.storage import DavStorage
...
async def test_ping_failure_with_cleanup_failure_preserves_client_and_error_order(
    ...
    monkeypatch.setattr("app.storage.dav.storage.AsyncDavClient", lambda _config: client)
...
async def test_retry_cleans_retained_client_before_replacing_it(
    ...
    monkeypatch.setattr("app.storage.dav.storage.AsyncDavClient", lambda _config: next(clients))
    storage = DavStorage(dav_config)
